# 재사용을 고려한다면 코드를 좀 고쳐야합니다.
# 일단은 임시로 필요한 데이터만 추출하기 위해 작성하였으니 참고부탁드립니다.
# 쓰레스홀드가 있는듯한데, 추후에 수정하겠습니다.
...
def parse_hotel(region_code, checkin_date, checkout_date):
    ...
    while True:
        ...
        for hotel in hotels['motels']['lists']:
            ...
            hotel_info[code] = {'grade':grade, 'name':name, 'address':address, 'location':location}
...
def parse_review(hotel_list):
    ...
    client=MongoClient('mongodb+srv://NLP:likelion@likelion.hppms.mongodb.net/myFirstDatabase?retryWrites=true&w=majority')
    db=client['NLP']
    collection=db['Nolja']  
    ...
    for code in list(hotel_list.keys()):
        url = 'https://domestic-order-site.yanolja.com/dos-server/review/properties/'+code+'/reviews'
...
if __name__ == "__main__":
    parse_hotel(region_code='900582', checkin_date='2021-05-24', checkout_date='2021-05-25')